...
(* TODO: prune the remaining of superseded material... *)
...
lemma val_mul_comba_upS k x y:
 ...
 = val_mul_comba_up k x y
   + bigi predT (fun (i : int) => to_uint x.[i] * to_uint y.[k - i] * W64.modulus ^ k)
          0 (k+1).
proof.
by move=> *; rewrite /val_mul_comba_up {1}(range_cat k) 1..2:/# rangeS big_cat big_cons big_nil /predT /=.
qed.
...
lemma val_mul_comba_downS k x y:
 ...
 = val_mul_comba_down k x y
   + bigi predT (fun (i : int) => to_uint x.[i] * to_uint y.[k - i] * W64.modulus ^ k)
          (1 + k - nlimbs) nlimbs.
proof.
by move=> *; rewrite /val_mul_comba_down (range_cat k) 1..2:/# rangeS big_cat big_cons big_nil /predT /#.
qed.
