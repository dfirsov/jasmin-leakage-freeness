...
(* We prove correctness of Jasmin's implementation of uniform sampling.  *)
...
rewrite /D. timeout 10. smt(@Distr @List).
...
equiv rsample_cspec:
 M.bn_rsample_i ~ CSpecFp.rsample:
...
lemma bn_rsample_spec &m a y: 0 <= W64xN.valR y < W64xN.valR a
  => Pr[ M.bn_rsample_i(a)@&m: res.`2 = y ] 
     = Pr[ RSM.RS.sample(fun x => x < W64xN.valR a,0)@&m: res.`2 = W64xN.valR y ].
  have ->: Pr[ M.bn_rsample_i(a)@&m: res.`2 = y ] = Pr[CSpecFp.rsample(W64xN.valR a) @ &m : res.`2 = W64xN.valR y ].
...
equiv rsample_aspec:
 M.bn_rsample_i ~ ASpecFp.rsample:
...
lemma bn_rsample_pmf &m a y: 0 <= W64xN.valR y < W64xN.valR a
  => Pr[ M.bn_rsample_i(a)@&m: res.`2 = y ] = 1%r/(W64xN.valR a)%r.
proof. move => interval.
